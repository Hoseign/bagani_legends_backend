...
export const equip_weapon_component = (() => {
  ...
  class EquipWeapon extends entity.Component {
    ...
    InitComponent() {
      ...
      this._RegisterHandler('player.action', (m) => this._OnAction(m));
    }
    ...
    _OnAction(msg) {
      if (this.target_) {
        this.target_.visible = (msg.action !== 'dance');
      }
    }

    _AttachTarget() {
      if (this._bones && this.target_) {
        const bone = this._bones[this.anchor_] || this._bones['RightHand'] || this._bones['mixamorigRightHand'];
        if (bone) {
          bone.add(this.target_);
        } else {
          console.error("Could not find suitable anchor bone for weapon attachment.");
        }
      }
    }
