...
class CrappyMMOAttempt {
  ...
  OnGameStarted_() {
    ...
    // Trigger UI transition immediately instead of waiting for 3D assets
    this.entityManager_.Get('ui').GetComponent('UIController').FadeoutLogin();
  ...
  Step_(timeElapsed) {
    const timeElapsedS = Math.min(1.0 / 10.0, timeElapsed * 0.001);
