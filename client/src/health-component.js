...
export const health_component = (() => {
  ...
  class HealthComponent extends entity.Component {
    ...
    OnNetworkUpdate_(msg) {
      // Block server updates for 1.5s after a local change (like respawn)
      if (Date.now() - this._lastNetUpdate < 1500) return;

      const s = msg.value;
      this.stats_.health = s.health[0];
      this.stats_.maxHealth = s.health[1];
