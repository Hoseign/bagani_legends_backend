...
export const sorceror_effect = (() => {
  ...
  class SorcerorEffect extends entity.Component {
    ...
    OnAttack_() {
      if (!this.params_.desc || this.params_.desc.character.class !== 'sorceror') {
        return;
      }

      const rightHand = this._bones['RightHandIndex1'] || this._bones['RightHand'] || this._bones['mixamorigRightHand'];
      const leftHand = this._bones['LeftHandIndex1'] || this._bones['LeftHand'] || this._bones['mixamorigLeftHand'];
      const hands = [rightHand, leftHand].filter(h => h);
