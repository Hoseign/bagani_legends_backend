...
export const player_entity = (() => {
  ...
  class BasicCharacterControllerProxy {
    constructor(animations, isLocal) {
  ...
  class BasicCharacterController extends entity.Component {
    ...
    ChangeClass(newClass) {
      ...
      const modelData = defs.CHARACTER_MODELS[newClass];
      this.Broadcast({
    ...
    LoadModels_() {
      ...
      if (!modelData) {
        console.error(`ERROR: Character model data not found for class: "${classType}". Falling back to zombie.`);
        this.params_.desc.character.class = 'zombie';
        this.LoadModels_();
        return;
      }
      ...
      const _OnLoad = (glb) => {
        // The FBX loader returns the group directly, GLTF returns an object containing the scene
        if (glb.scene) {
        ...
        this.stateMachine_ = new CharacterFSM(
            new BasicCharacterControllerProxy(this.animations_, true));
